...
import {HelperConfigVault} from "./HelperConfigVault.s.sol";
...
contract DeployVault is Script {
    ...
    function resolvedTokenAddressForRun() external returns (address tokenAddress) {
        ...
        HelperConfigVault.VaultDeployConfig memory config = helperConfig
     ...
     * @return tokenAddress The resolved vault token address.
     */
    function resolvedTokenAddressForRunWithPrefix(string calldata envPrefix)
        external
        returns (address tokenAddress)
    {
        HelperConfigVault helperConfig = new HelperConfigVault();
        HelperConfigVault.VaultDeployConfig memory config = helperConfig
            .getActiveNetworkConfigWithPrefix(envPrefix);
     ...
     * @return vault The deployed VerifundBasicVault instance.
     */
    function runWithEnvPrefix(string calldata envPrefix)
        external
        returns (VerifundBasicVault vault)
    {
        HelperConfigVault helperConfig = new HelperConfigVault();
        HelperConfigVault.VaultDeployConfig memory config = helperConfig
            .getActiveNetworkConfigWithPrefix(envPrefix);
     ...
     * @return vault The deployed VerifundBasicVault instance.
     */
    function _deploy(HelperConfigVault.VaultDeployConfig memory config)
        internal
        returns (VerifundBasicVault vault)
    {
        vm.startBroadcast();
