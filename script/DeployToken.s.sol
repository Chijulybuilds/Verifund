...
import {HelperConfig} from "./HelperConfigToken.s.sol";
...
contract DeployToken is Script {
    ...
    function run() external returns (SecureERC223Token token) {
        ...
        NetworkConfigModule.TokenDeployConfig memory config = helperConfig
    ...
    function runWithEnvPrefix(string calldata envPrefix)
        external
        returns (SecureERC223Token token)
    {
        HelperConfig helperConfig = new HelperConfig();
        NetworkConfigModule.TokenDeployConfig memory config = helperConfig
     ...
     * @return token The deployed SecureERC223Token instance.
     */
    function _deploy(NetworkConfigModule.TokenDeployConfig memory config)
        internal
        returns (SecureERC223Token token)
    {
        vm.startBroadcast();
