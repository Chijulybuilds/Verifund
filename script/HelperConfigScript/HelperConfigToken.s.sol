...
contract HelperConfig is Script {
     ...
     */
    function getActiveNetworkConfig() public view returns (NetworkConfigModule.TokenDeployConfig memory config) {
     ...
     */
    function _applyEnvOverrides(NetworkConfigModule.TokenDeployConfig memory config, string memory prefix)
        internal
        view
        returns (NetworkConfigModule.TokenDeployConfig memory)
    {
        string memory scopedPrefix = bytes(prefix).length == 0 ? "TOKEN" : string.concat(prefix, "_TOKEN");

        config.name = vm.envOr(string.concat(scopedPrefix, "_NAME"), config.name);
        config.symbol = vm.envOr(string.concat(scopedPrefix, "_SYMBOL"), config.symbol);
        config.initialSupply = vm.envOr(string.concat(scopedPrefix, "_INITIAL_SUPPLY"), config.initialSupply);

        uint256 decimalsOverride = vm.envOr(string.concat(scopedPrefix, "_DECIMALS"), uint256(config.decimals));
